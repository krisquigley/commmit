...
module DashboardHelper
  def group_tickets(tickets)
    tickets.map do |repo, repo_records|
      ...
      average_lead_time = repo_tickets.transform_values do |records|
        average_lead_time_in_seconds(records)
      end.sort
  ...
  def average_lead_time_in_seconds(records)
    ...
    { estimated_time: average_estimated_seconds, assigned_time: average_assigned_seconds,
      record_count: count }
  end

  def record_created_at(record)
    (JSON.parse(record.source)['created_at'] ||
      JSON.parse(record.source)['issue']['created_at']).to_time
  end
end
