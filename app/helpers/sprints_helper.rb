...
module SprintsHelper
  ...
  def effort_to_date(sprint)
    ...
    if remaining_effort = calculate_effort_done_before_start_date(merged_tickets,
                                                                  sprint.start_date.to_date)
      current_effort -= remaining_effort
    end

    while day <= Date.today && day <= sprint.end_date
      tickets = merged_tickets.find_all do |merged_ticket|
        day == merged_ticket.closed_at.to_date
      end

      current_effort -= tickets.map(&:estimated_effort).reduce(:+) unless tickets.empty?
  ...
  def average_velocity_per_person_per_day_half_sprint(current_sprint, velocity_per_person_per_day)
    half_a_sprint = current_sprint.sprint_length_in_days / 2
    count = velocity_per_person_per_day.count
    if count.positive?
      total_velocity_per_person_per_day = velocity_per_person_per_day
                                          .map(&:velocity_per_person_per_day).reduce(&:+)
  ...
  def calculate_effort_done_before_start_date(merged_tickets, start_date)
    ...
    remaining_tickets ? remaining_tickets.map(&:estimated_effort).reduce(:+) : nil
  end
end
