...
class SprintsController < ApplicationController
  protect_from_forgery except: %i[export_csv download_csv]
  ...
  def show
    @sprint = Sprint.includes(:sprint_tickets).order('sprint_tickets.position asc')
  ...
  def manage
    ...
    @tickets = Ticket.where.not(issue_id: @sprint_tickets.pluck(:issue_id))
                     .where(closed_at: nil)
                     .order(updated_at: :desc)
                     .page(params[:page])

    @recent_velocity_per_person_per_day = @sprint.team.sprints
                                                 .where.not(final_velocity: nil)
                                                 .order(end_date: :desc)
                                                 .limit(3)

    if (params[:search] && !params[:search].empty?) &&
       (params[:repository_name] && !params[:repository_name].empty?)
      @tickets = @tickets.search_by_title_or_issue_number_and_filter(params[:search],
