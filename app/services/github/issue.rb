...
module Github
  class Issue < Github::Base
    ...
    def repository_name
      parsed_payload[:repository].to_h[:full_name] || parsed_payload.fetch(:html_url)[
%r{github.com/(.*)/issues}, 1]
    end

    def estimated_effort
      @estimated_effort ||= begin
        Float((parsed_payload[:issue].to_h[:title] || parsed_payload.fetch(:title))[/\[(.*)\]/,
                                                                                    1])
