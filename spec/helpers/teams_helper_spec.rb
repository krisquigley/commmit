...
RSpec.describe TeamsHelper do
  describe '#happiness_values' do
    ...
    let!(:sprint_with_tickets) do
      user = create(:user)
      team.users << user
      sprint_with_tickets = create(:sprint_with_tickets, team: team, start_date: '2019-01-01',
                                                         end_date: '2019-01-05')
      sprint_with_tickets.update(closed_at: '2019-01-05')

      sprint_with_tickets.team.users.each do |team_user|
        sprint_with_tickets.retrospectives.create!(user: team_user,
                                                   role_happiness: 1,
                                                   company_happiness: 4,
                                                   feedback: 'test',
                                                   happiness_goal: 'test')
    ...
    let!(:sprint_with_tickets_2) do
      sprint_with_tickets = create(:sprint_with_tickets, team: team, start_date: '2019-01-08',
                                                         end_date: '2019-01-12')
      sprint_with_tickets.update(closed_at: '2019-01-12')
      sprint_with_tickets.team.users.each do |user|
        sprint_with_tickets.retrospectives.create!(user: user,
                                                   role_happiness: 3,
                                                   team_happiness: 5,
                                                   company_happiness: 2,
                                                   feedback: 'test',
    ...
    it 'should return json in the correct format' do
      expect(helper.happiness_values(sprints))
        .to eq '[{"end_date":"2019-01-05T00:00:00.000Z","ave'\
        'rage_happiness":2.5},{"end_date":"2019-01-12T00:00:00.000Z","average_happiness":3.3}]'
    end
  end
  ...
  describe '#velocity' do
    ...
    it 'should return final velocity and end date in json' do
      ...
      expect(JSON.parse(helper.velocity(closed_sprints_with_closed_tickets)).last).to eq(
        { 'end_date' => expectation.end_date.strftime('%Y-%m-%dT%H:%M:%S.%LZ'),
          'final_velocity' => expectation.velocity.to_f.round(1).to_s, 'id' => nil }
      )
    end
  end
  ...
  describe '#no_of_members_per_sprint' do
    ...
    it 'should return an array in json' do
      expect(helper.no_of_members_per_sprint(closed_sprints_with_closed_tickets))
        .to eq '[2,2,2,2,3]'
    end
  end
  ...
  describe '#velocity_per_person_per_day_per_sprint' do
    ...
    it 'should return an array in json' do
      expect(helper.velocity_per_person_per_day_per_sprint(closed_sprints_with_closed_tickets))
        .to be_a String

      expectation = Sprint.order(end_date: :desc).first.velocity / (team.users.count * 5.0)
      expect(JSON.parse(helper
        .velocity_per_person_per_day_per_sprint(closed_sprints_with_closed_tickets))
          .last).to eq expectation.round(2).to_s
    end
  end
end
